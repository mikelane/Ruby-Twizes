...
configInfo = Psych.load_file("config.yaml")
configInfo = Psych.load(configInfo)

# Configure the TweetStream API
TweetStream.configure do |config|
  config.consumer_key       = configInfo['consumer_key']
  config.consumer_secret    = configInfo['consumer_secret']
  config.oauth_token        = configInfo['oauth_token']
  config.oauth_token_secret = configInfo['oauth_token_secret']
  ...
  if !original_tweet.nil?
    ...
    if retweets[original_tweet.id].nil?
      ...
      if original_tweet.text.downcase.include? "win "
        ...
        if original_tweet.text.downcase.include? "follow"
          puts "Follow:  [@#{original_tweet.user.screen_name}]"
        end
